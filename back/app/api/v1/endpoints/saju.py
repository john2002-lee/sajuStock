...
`POST /saju/shares` 는 여덟 글자와 무료 요약을 7일간 저장한다. 결과를 링크로
보내려면 링크가 가리킬 것이 있어야 하기 때문이다. **이 파일이 아니라
`saju_shares.py` 에 있다** — 위 문장을 거짓으로 만들지 않기 위해서다. 그 예외가
어디까지인지(무엇을 담지 않는지, 왜 7일인지)는 그 파일과 `models/saju_share.py`
모듈 주석에 있다.

생년월일시는 그쪽에서도 **저장되지 않는다.** 계산에만 쓰고 버린다.
