 ...
 *
 * "당신의 사주팔자, 시간부터 바로잡습니다" 는 폭에 따라 갈라지는 자리가 달라진다.
 * `word-break: keep-all`(globals.css 의 사주 스코프)이 어절 중간에서 끊는 것은
 * 막지만, 그래도 쉼표를 무시한 자리에서 갈릴 수 있다. 히어로 문구는 **의도한
 * 자리에서만** 끊겨야 하므로 두 줄을 각각 블록으로 둔다 — 어느 폭에서도 같은
 * 모양이다.
 *
 * ## 왜 "정확하게 읽습니다" 가 아닌가
 *
 * 그것이 원래 문구였고, 이 화면에서 가장 큰 글자였다. 세 가지가 걸렸다.
 *
 *  1. **약관이 반대로 말한다.** 제8조는 "내용의 정확성이나 특정 결과를 보장하지
 *     않습니다" 라고 적고 있다. 제품의 첫 화면과 약관이 서로 다른 말을 하고 있었다.
 *  2. **소개 페이지는 이미 그 규칙을 지키고 있었다** — `IntroStory` 머리말이
 *     후기·이용자 수·"가장 정확한" 류를 쓰지 않는다고 못박아 두었는데, 정작 제일
 *     큰 글자에는 적용되지 않았다. 로또 쪽(`LottoSection`)도 표시광고법을 이유로
 *     같은 규칙을 적어 두고 있다.
 *  3. 사주의 "정확함" 은 검증할 방법이 없다. 반면 **진태양시 보정은 실제로 하는
 *     일**이고, 부제와 소개 페이지가 그 이야기를 한다. 할 수 있는 말로 바꾼다.
 */
      ...
      <div className="grid gap-y-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,420px)] lg:items-start lg:gap-x-16">
        <header className="text-center lg:col-start-1 lg:row-start-1 lg:pt-2 lg:text-left">
          ...
          <h1 className="mt-3 font-display text-[30px] font-light leading-[1.25] text-ink sm:text-[38px] lg:text-[44px]">
            ...
            <span className="block text-gold-text">시간부터 바로잡습니다</span>
          </h1>
