 ...
 *
 * ## 읽기 전용이라는 것은 슬롯을 비우는 것으로 표현된다
 *
 * `WebtoonReport` 를 그대로 쓰되 `followUp` 과 `share` 를 **넘기지 않는다.** 본문과
 * 계산 패널은 구매자가 보는 것과 같고, 질문 입력창과 공유 버튼만 없다.
 *
 ...
 * 좁혀 내리므로 생년월일·진태양시 보정 분값·추가 질문·접근 토큰이 응답에 없고,
 * 타입(`SharedReport.chart` 는 `ReportChart`)이 그 사실을 붙들고 있다. 화면에서만
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-6">
      ...
      <p className="text-center text-[11.5px] leading-relaxed text-muted-2">
        ...
        <br />
        공유된 것은 풀이와 계산 결과뿐이며, 보낸 분의 생년월일과 추가 질문은 담기지 않습니다.
      </p>
    </div>
  );
}
